...
function raycaster(x, y, alpha, pointerAlpha, accumulatedJumpHeight) {
    ...
    alpha += Math.PI * 0.5;
    //
    const hitCoordinate = loop(Math.sin(alpha), Math.cos(alpha), x, y, x, y, mapWidth, mapHeight, alpha, pointerAlpha, []);
...
function loop(sin, cos, x, y, originX, originY, mapWidth, mapHeight, alpha, pointerAlpha, array) {
    let
        nextX = sin > 0
    ...
    if (checkHitResult !== 999) {
        return loop(sin, cos, x, y, originX, originY, mapWidth, mapHeight, alpha, pointerAlpha, array);
    } else {
        return array
    }
}
